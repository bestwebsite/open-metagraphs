...
Bestwebsite_Open_Graph_Meta_Tags::init();

/**
 * Bestwebsite Open Graph Meta Tags
 *
 * @package      Bestwebsite_Open_Graph_Meta_Tags
 * @author       Bestwebsite
 */
class Bestwebsite_Open_Graph_Meta_Tags {
	...
	public static function get_meta() {
		...
		$output = apply_filters( 'Bestwebsite_open_graph_meta_tags', $output );
	...
	private static function get_meta_home() {
		...
		return apply_filters( 'Bestwebsite_open_graph_meta_tags_home', $output );
	}
	...
	private static function get_meta_author() {
		...
		$output = apply_filters( 'Bestwebsite_open_graph_meta_tags_author', $output, $author );
		if ( isset( $author->ID ) )
			$output = apply_filters( "Bestwebsite_open_graph_meta_tags_author_{$author->ID}", $output, $author );
	...
	private static function get_meta_term() {
		...
		$output = apply_filters( 'Bestwebsite_open_graph_meta_tags_term', $output, $term );
		if ( isset( $term->taxonomy ) )
			$output = apply_filters( "Bestwebsite_open_graph_meta_tags_term_{$term->taxonomy}", $output, $term );
	...
	private static function get_meta_singular() {
		...
		if ( post_type_supports( $post_type, 'thumbnail' ) ) {
			$post_thumbnail_id = absint( apply_filters( 'Bestwebsite_open_graph_meta_tags_default_image_id', 0 ) );
		...
		$output = apply_filters( 'Bestwebsite_open_graph_meta_tags_singular', $output, $post, $post_type );
		$output = apply_filters( "Bestwebsite_open_graph_meta_tags_singular_{$post_type}", $output, $post, $post_type );
